...
layout: about
category: about
Researchname: 「poseidon3」：防护Token泄露（全微服务）
toc: true
author: Bin4xin
...
| **发现日期** | 2026-07-07 |
| **影响范围** | <span style="color:#cf222e;font-weight:bold">🔴 全部10个微服务</span> |
...
{% include common-index/index-preset.html level="warn" msg="防护Token获取接口无需认证即可访问，导致整个平台的防Burp抓包机制失效，攻击者可零成本获取所有微服务的防护Token。" %}
...
{% include common-index/index-preset.html level="info" msg="提示：`/inner/` 路径接口在设计上属于内部接口，但由于未配置网络隔离或认证拦截，所有内部接口均暴露在公网，建议全面排查。" %}
...
| 字段 | 内容 | 风险等级 |
|------|------|----------|
| `code` | 状态码 `00000` 表示成功 | <span style="color:#1f7bdf">🔵 低</span> |
| `data` | 防护Token数组，包含MD5格式的Token值 | <span style="color:#cf222e;font-weight:bold">🔴 高</span> |
...
{% include common-index/index-preset.html level="warn" msg="注意：验证过程应在授权范围内进行，请勿将获取的Token用于未授权的渗透测试或对生产环境的攻击。" %}
...
{% include common-index/index-preset.html level="warn" msg="攻击链说明：防护Token的获取无需任何认证，攻击者可自动化遍历所有微服务获取Token后，结合代理工具对整个平台进行无限制的渗透测试。" %}
...
{% include common-index/index-preset.html level="info" msg="提示：根因在于防护机制的设计缺陷——Token获取与校验分离，导致获取Token的接口本身成为绕过防护的入口。建议从根本上重构防护架构。" %}
...
| 维度 | 影响 |
|------|------|
| **机密性** | <span style="color:#bc6f00">🟡 中</span> — 绕过防护后可使用代理工具抓取请求数据 |
| **完整性** | <span style="color:#bc6f00">🟡 中</span> — 绕过防护后可重放和篡改请求 |
| **可用性** | <span style="color:#1f7bdf">🔵 低</span> — 不直接影响系统可用性 |
| **业务影响** | <span style="color:#cf222e;font-weight:bold">🔴 高</span> — 整个平台的防抓包防护机制失效 |
| **攻击面** | <span style="color:#cf222e;font-weight:bold">🔴 极广</span> — 影响全部10个微服务 |
...
{% include common-index/index-preset.html level="error" msg="紧急：防护Token机制对所有10个微服务均失效，建议在发现漏洞后 24 小时内完成 P0 级修复。" %}
...
{% include common-index/index-preset.html level="info" msg="提示：P1 级修复建议在漏洞发现后 7 个工作日内完成，并在修复完成后进行回归测试，确认防护机制功能正常。" %}
